...
export const AccountManager = ({ journals, activeJournalId, setActiveJournalId, user, showAlert, db }: AccpountManagerProps) => {
    ...
    const handleSaveJournal = async () => {
        ...
        if (editingJournal) {
            if (!appId) return;
            await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'journals', editingJournal.id), { name: journalName });
        } else {
            if (!appId) return;
            await addDoc(collection(db, 'artifacts', appId, 'users', user.uid, 'journals'), { name: journalName, createdAt: serverTimestamp(), balance, dailyProfitTarget: 0, weeklyProfitGoal: 0, riskPercentage: 2 });
        }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 space-y-4">
                {journals.map(journal => (<div key={journal.id} className={`flex items-center justify-between p-4 rounded-lg ${activeJournalId === journal.id ? 'bg-blue-100 dark:bg-blue-500/20 border-blue-500 border-2' : 'bg-gray-50 dark:bg-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-700'}`}><button onClick={() => handleSwitchClick(journal)} className="flex-1 text-left"><h3 className="font-semibold text-lg">{journal.name}</h3><p className="text-sm text-gray-600 dark:text-gray-400">Balance: ${journal.balance?.toFixed(2)}</p></button><div className="flex items-center space-x-2"><button onClick={() => confirmResetJournal(journal)} className="p-2 text-gray-500 dark:text-gray-400 hover:text-yellow-500 rounded-full" title="Reset Journal"><RefreshCw className="w-5 h-5" /></button><button onClick={() => handleOpenModal(journal)} className="p-2 text-gray-500 dark:text-gray-400 hover:text-blue-600 rounded-full" title="Edit Journal"><Edit className="w-5 h-5" /></button><button onClick={() => confirmDeleteJournal(journal.id, journal.name)} className="p-2 text-gray-500 dark:text-gray-400 hover:text-red-600 rounded-full" title="Delete Journal"><Trash2 className="w-5 h-5" /></button></div></div>))}
            </div>
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingJournal ? "Edit Journal" : "Create New Journal"}><div className="space-y-4"><label className="block"><span className="text-gray-600 dark:text-gray-300">Journal Name</span><input type="text" value={journalName} onChange={(e) => setJournalName(e.target.value)} className="w-full bg-gray-100 dark:bg-gray-700 p-3 rounded-lg mt-1" /></label>{!editingJournal && (<label className="block"><span className="text-gray-600 dark:text-gray-300">Initial Balance ($)</span><input type="number" value={initialBalance} onChange={(e) => setInitialBalance(e.target.value)} className="w-full bg-gray-100 dark:bg-gray-700 p-3 rounded-lg mt-1" /></label>)}<button onClick={handleSaveJournal} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-lg">{editingJournal ? 'Save Changes' : 'Create Journal'}</button></div></Modal>
