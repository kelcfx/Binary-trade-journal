...
interface TradeData {
    ...
    time: string;
    sessionProfit?: number | string;
    investmentPerTrade?: number | string;
    roi?: number | string;
    ...
    return (
        <form onSubmit={onSubmit} className="space-y-4">
            ...
            {isManualProfit ? (
                ...
                <>
                    <div className="grid grid-cols-2 gap-4">
                        <input type="number" name="investmentPerTrade" value={tradeData.investmentPerTrade} onChange={onInputChange} placeholder="Investment per Trade ($)" required className="w-full bg-gray-100 dark:bg-gray-700 p-3 rounded-lg" />
                        <input type="number" name="roi" value={tradeData.roi} onChange={onInputChange} placeholder="ROI (%)" required className="w-full bg-gray-100 dark:bg-gray-700 p-3 rounded-lg" />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <input type="number" name="totalTrades" value={tradeData.totalTrades} onChange={onInputChange} placeholder="Total Trades" required className="w-full bg-gray-100 dark:bg-gray-700 p-3 rounded-lg" />
                        <input type="number" name="losingTrades" value={tradeData.losingTrades} onChange={onInputChange} placeholder="Losing Trades" required className="w-full bg-gray-100 dark:bg-gray-700 p-3 rounded-lg" />
                    </div>
                </>
            )}
            <select name="direction" value={tradeData.direction} onChange={onInputChange} className="w-full bg-gray-100 dark:bg-gray-700 p-3 rounded-lg">
                <option>Buy</option>
                <option>Sell</option>
            </select>
